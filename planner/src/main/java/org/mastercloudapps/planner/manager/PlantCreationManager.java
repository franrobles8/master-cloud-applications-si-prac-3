...
public class PlantCreationManager {
	...
	public void process() {
		CompletableFuture<TopoResponse> topography = topoService.getLandscape(city);
		CompletableFuture<Weather> weather = weatherService.getWeather(city);
	...
	private PlantCreationProgress getCreationProgress() {
		...
		PlantCreationProgress message = new PlantCreationProgressBuilder().id(id).city(city).progress(progress)
				.planning(planning).completed(completed).build();

		if (completed) {
			int randomProcessTime = new Random().ints(MIN_WAIT, MAX_WAIT + 1).findFirst().getAsInt();
			log.info("Random process time for completed plant creation for id {} is {} second(s)", id,
					randomProcessTime);
			try {
